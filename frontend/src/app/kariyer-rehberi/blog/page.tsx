...
import { blogPosts, blogCategories, filterAndSortContent, formatDate } from "@/lib/ContentTypes"
...
export default function BlogPage() {
  ...
  const filteredBlogs = filterAndSortContent(blogPosts, searchQuery, selectedCategory, sortBy)
  ...
  return (
    <div className="page-content mx-auto  py-8">
      ...
      <h1 className="text-3xl font-bold py-4">Blog Yazıları</h1>
...
function ContentCard({ title, description, footer, category, popularity }: ContentCardProps) {
  return (
    <Link href={`/kariyer-rehberi/blog/${title.replace(/\s+/g, "-").toLowerCase()}`} className="w-full">
